...
export async function requestKybDocumentsAction({
	...
	try {
		...
		if (recipients.length === 0) {
			return {
				success: false,
				error: "No organization owners or admins found to notify.",
			};
		}
...
export async function listJobQueueAdminAction(
	input: ListJobQueueAdminInput = {},
) {
	await requireAdmin();
	...
	try {
		...
		return {
			success: false as const,
			error: error?.message || "Failed to fetch job queue entries.",
		};
	}
}
...
export async function requeueJobAdminAction(jobId: string) {
	...
	try {
		...
		return {
			success: true as const,
			message: `Re-queued job ${jobId} (${job.type})`,
		};
	} catch (error: any) {
		console.error("[requeueJobAdminAction] Error:", error);
		return {
			success: false as const,
			error: error?.message || "Failed to re-queue job.",
		};
	}
}
...
export async function requeueAllFailedJobsAdminAction(jobType?: string) {
	...
	try {
		...
		return {
			success: true as const,
			count: result.count,
			message: `Re-queued ${result.count} failed job(s)`,
		};
	} catch (error: any) {
		console.error("[requeueAllFailedJobsAdminAction] Error:", error);
		return {
			success: false as const,
			error: error?.message || "Failed to re-queue jobs.",
		};
	}
}
