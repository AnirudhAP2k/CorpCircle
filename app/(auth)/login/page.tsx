...
export default function LoginPage() {
    return (
        <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-8 items-center max-w-6xl mx-auto my-auto">
            ...
            <div className="lg:col-span-7 relative rounded-2xl overflow-hidden min-h-[280px] md:min-h-[480px] lg:min-h-[580px] flex flex-col justify-end p-6 sm:p-8 md:p-12 shadow-nx-float border border-nx-on-primary/10 group">
                {/* Background Image */}
                <Image
                ...
                <div className="relative z-20 space-y-6">
                    ...
                    <div className="flex flex-wrap gap-3 pt-2">
                        {LOGIN_HIGHLIGHTS.map((item) => (
                            <div key={item.label} className="inline-flex items-center gap-2.5 bg-nx-on-primary/10 backdrop-blur-md rounded-xl px-4 py-2.5 border border-nx-on-primary/10">
                                <item.icon className="w-4 h-4 text-nx-on-tertiary-container shrink-0" />
                                <span className="text-xs font-label font-semibold text-nx-on-primary">{item.label}</span>
                            </div>
                        ))}
                    </div>
                </div>
