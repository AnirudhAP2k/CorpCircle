...
const INTENT_BADGES: Record<string, { label: string; color: string }> = {
	...
	SEEKING_CLIENTS: {
		...
		color:
			"bg-nx-success-container text-nx-on-success-container border-nx-success/20",
	},
	SEEKING_VENDORS: {
		label: "Seeking Vendors",
		color:
			"bg-nx-tertiary-container text-nx-on-tertiary-container border-nx-tertiary/20",
	},
	SEEKING_INVESTMENT: {
	...
	SPONSORING_EVENTS: {
		...
		color:
			"bg-nx-error-container text-nx-on-error-container border-nx-error/20",
	},
};
...
const ChipList = ({
	items,
	colorClass = "bg-nx-secondary-container/60 text-nx-on-secondary-container border-nx-secondary/20",
}: {
	items: string[];
	colorClass?: string;
}) => {
	if (!items || items.length === 0)
		return (
			<p className="text-sm text-nx-on-surface-variant italic">None listed</p>
		);
	return (
...
const OrganizationProfilePage = async ({
	...
	if (!isMember) {
		...
		if (activeOrgIdForButton) {
			...
			if (!conn) {
				...
				connectionStatus =
					conn.sourceOrgId === activeOrgIdForButton
						? "PENDING_SENT"
						: "PENDING_RECEIVED";
	...
	return (
		<div className="min-h-screen bg-nx-surface font-body text-nx-on-surface">
			...
			{/* Hero / header band */}
			<div className="bg-nx-surface-container-lowest border-b border-nx-outline-variant/40">
				<div className="wrapper py-8">
					<div className="flex flex-col md:flex-row items-start gap-6">
						{/* Logo */}
						<div className="relative w-20 h-20 flex-shrink-0 rounded-xl overflow-hidden bg-nx-surface-container border border-nx-outline-variant shadow-sm">
							{organization.logo ? (
								<Image
								...
								<div className="w-full h-full flex items-center justify-center">
									<Building2 className="w-10 h-10 text-nx-on-surface-variant/50" />
								</div>
							)}
						</div>
						...
						<div className="flex-1 min-w-0">
							<div className="flex items-center gap-2 flex-wrap mb-1">
								<h1 className="text-2xl font-headline font-bold text-nx-on-surface">
									{organization.name}
								</h1>
								{organization.isVerified && (
									<CheckCircle
										className="w-5 h-5 text-nx-primary flex-shrink-0"
										aria-label="Verified organization"
									/>
								)}
							</div>
							<div className="flex flex-wrap items-center gap-3 text-sm text-nx-on-surface-variant mb-3">
								<span>{organization.industry.label}</span>
								{organization.size && (
									<Badge
										variant="outline"
										className="text-xs border-nx-outline-variant text-nx-on-surface-variant"
									>
							...
							<div className="flex items-center gap-x-3 gap-y-2 flex-wrap min-w-0">
								...
								{(organization as any).linkedinUrl && (
									<a
										href={(organization as any).linkedinUrl}
										target="_blank"
										rel="noopener noreferrer"
										className="flex items-center gap-1.5 text-sm text-nx-on-surface-variant hover:text-nx-tertiary transition-colors"
									>
								...
								{(organization as any).twitterUrl && (
									<a
										href={(organization as any).twitterUrl}
										target="_blank"
										rel="noopener noreferrer"
										className="flex items-center gap-1.5 text-sm text-nx-on-surface-variant hover:text-nx-tertiary transition-colors"
									>
						...
						{/* Action buttons */}
						<div className="flex w-full items-center gap-2 flex-wrap md:w-auto md:flex-shrink-0">
				...
				<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
					...
					<div className="space-y-6 lg:col-span-2 min-w-0">
						...
						<Card className="rounded-2xl border-nx-outline-variant/60 bg-nx-surface-container-lowest shadow-nx-card">
							<CardHeader>
								<CardTitle className="font-headline text-nx-on-surface">
									About
								</CardTitle>
							</CardHeader>
							<CardContent>
								{organization.description ? (
									<p className="text-nx-on-surface leading-relaxed">
										{organization.description}
									</p>
								) : (
									<p className="text-nx-on-surface-variant italic">
										No description provided
									</p>
								)}
								<div className="flex flex-wrap gap-6 text-sm text-nx-on-surface-variant mt-4 pt-4 border-t border-nx-outline-variant/40">
									<span>
										<strong>{organization._count.members}</strong> Members
									</span>
									<span>
										<strong>{organization._count.events}</strong> Events Hosted
									</span>
								</div>
							</CardContent>
						</Card>
						...
						<Card className="rounded-2xl border-nx-outline-variant/60 bg-nx-surface-container-lowest shadow-nx-card">
							<CardHeader>
								<CardTitle className="flex items-center gap-2">
									<Briefcase className="w-4 h-4 text-nx-on-surface-variant" />
									Services Offered
								</CardTitle>
							</CardHeader>
							<CardContent>
						...
						<Card className="rounded-2xl border-nx-outline-variant/60 bg-nx-surface-container-lowest shadow-nx-card">
							<CardHeader>
								<CardTitle className="flex items-center gap-2">
									<Code2 className="w-4 h-4 text-nx-on-surface-variant" />
									Technologies
								</CardTitle>
							</CardHeader>
							<CardContent>
								<ChipList
									...
									colorClass="bg-nx-tertiary-container text-nx-on-tertiary-container border-nx-tertiary/20"
								/>
							</CardContent>
						</Card>
						...
						<Card className="rounded-2xl border-nx-outline-variant/60 bg-nx-surface-container-lowest shadow-nx-card">
							<CardHeader>
								<CardTitle className="flex items-center gap-2">
									<Handshake className="w-4 h-4 text-nx-on-surface-variant" />
									Partnership Interests
								</CardTitle>
							</CardHeader>
							<CardContent>
							...
							<Card className="rounded-2xl border-nx-outline-variant/60 bg-nx-surface-container-lowest shadow-nx-card">
								...
								<CardContent>
									<ChipList
										...
										colorClass="bg-nx-surface-container-high text-nx-on-surface border-nx-outline-variant"
									/>
								</CardContent>
							</Card>
						)}
						...
						<Card className="rounded-2xl border-nx-outline-variant/60 bg-nx-surface-container-lowest shadow-nx-card">
							<CardHeader>
								<CardTitle className="font-headline text-nx-on-surface">
									Hosted Events ({organization._count.events})
								</CardTitle>
								<CardDescription>
									Events organized by this organization
								</CardDescription>
							</CardHeader>
							<CardContent>
								{organization.events.length > 0 ? (
									<div className="grid gap-4 sm:grid-cols-2">
										{organization.events.map((event) => (
											<Link
												...
												className="block border border-nx-outline-variant/60 rounded-xl overflow-hidden bg-nx-surface-container-lowest hover:shadow-md transition-shadow"
											>
												{event.image && (
												...
												<div className="p-4">
													<h3 className="font-headline font-semibold mb-1 truncate">
														{event.title}
													</h3>
									...
									<div className="text-center py-8">
										<Calendar className="w-10 h-10 text-nx-on-surface-variant/50 mx-auto mb-3" />
					...
					<div className="space-y-6 min-w-0">
						...
						<Card className="rounded-2xl border-nx-outline-variant/60 bg-nx-surface-container-lowest shadow-nx-card">
							...
							<CardContent className="space-y-3 text-sm">
								<div className="flex justify-between gap-4 text-nx-on-surface-variant">
									<span>Industry</span>
									<span className="font-medium text-right text-nx-on-surface">
								...
								<div className="flex justify-between gap-4 text-nx-on-surface-variant">
									<span>Intent</span>
									<span
										className={`text-xs font-medium px-2 py-0.5 rounded-full border ${intentBadge.color}`}
									>
										{intentBadge.label}
									</span>
								</div>
