...
const billingOrgPort = (org: BillingOrgRecord): BillingOrg => {
	return {
		id: org.id,
		name: org.name,
...
const resolveCheckoutCurrency = (
	...
	const stored = isBillingCurrency(org.preferredCurrency)
...
const assertCurrencyEligibility = (
	org: BillingOrgRecord,
...
export async function createBillingCheckout(input: {
	...
	if (input.plan === "ENTERPRISE") {
		throw new BillingError(
			400,
			"Enterprise is billed through sales. Use the contact form.",
		);
	}
	...
	const activePaid = isCurrencyLocked(
	...
	if (
		...
		org.preferredCurrency !== currency
	) {
...
export async function cancelOrgSubscription(userId: string): Promise<void> {
	...
	const provider: PaymentProvider =
		sub.provider === "RAZORPAY" ? "razorpay" : "stripe";
	const gateway = getPaymentGateway(provider);
...
export async function getBillingStatus(userId: string): Promise<BillingStatus> {
	...
	return {
		...
		preferredCurrency: isBillingCurrency(org.preferredCurrency)
			? org.preferredCurrency
			: "USD",
